...
let rec eval_exp = function
  ...
  | S.Match (e, e1, x1, x2, e2) ->
      begin match eval_exp e with
      | S.Nil -> eval_exp e1
      | S.Cons (e3, e4) -> eval_exp (S.subst [(x1, e3); (x2, e4)] e2)
...
let rec step = function
  | S.Var _ | S.Int _ | S.Bool _ | S.Lambda _ | S.RecLambda _ | S.Pair _ | S.Cons _ | S.Nil -> failwith "Expected a non-terminal expression"
  ...
  | S.Match (v, e1, x1, x2, e2) when is_value v ->
      begin match v with
      | S.Nil -> e1
      | Cons (e3, e4) -> S.subst [(x1, e3); (x2, e4)] e2
      | _ -> failwith "List expected"
      end
