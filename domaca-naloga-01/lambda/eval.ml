...
let rec eval_exp = function
  ...
  | S.Cons (e1, e2) ->
      ...
      and v2 = eval_exp e2
      in S.Cons (v1, v2)
  | S.Match (e, e1, x1, x2, e2) ->
      begin match eval_exp e with
      | S.Nil -> eval_exp e1
      | S.Cons (v1, v2) -> eval_exp (S.subst [(x1, v1); (x2, v2)] e2)
...
let rec is_value = function
  ...
  | S.Pair ( x1, x2 ) -> ( is_value x1 ) && ( is_value x2 )
  | S.Cons ( x1, x2 ) -> is_value x1 && is_value x2
...
let rec step = function
  ...
  | S.Pair (v1, e1) when is_value v1 -> S.Pair (v1, step e1)
  ...
  | S.Match (v, e1, x1, x2, e2) when is_value v ->
      begin match v with
      | S.Nil -> e1
      | Cons (v1, v2) -> S.subst [(x1, v1); (x2, v2)] e2
      | _ -> failwith "List expected"
      end
  | S.Match (e, e1, x1, x2, e2) -> S.Match (step e, e1, x1, x2, e2)
